...
class MathProblemCache:
        ...
        if self.use_cached_problems:
            ...
            if self.use_sqlite:
                async with aiosqlite.connect(self.db_name) as conn:
                    ...
                    cursor = await conn.cursor()
                    log.debug(f"Getting the problem with guild id {guild_id} and problem_id {problem_id} (types: {type(guild_id)}, {type(problem_id)})")
                    await cursor.execute(
                        "SELECT * FROM problems WHERE guild_id = ? AND problem_id = ?", # Not sure if making "from" lowercase will change anything (but it selects the problem from the database)
                        (guild_id, problem_id), 
                    )
    ...
    async def update_problem(
            self, guild_id: typing.Optional[int], problem_id: int, new: BaseProblem
    ) -> None:
