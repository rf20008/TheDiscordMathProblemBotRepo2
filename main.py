...
async def on_slash_command_error(inter, error, print_stack_traceback=[True, stderr]):
    ...
    await inter.reply(embed=ErrorEmbed(nextcord.utils.escape_markdown(error_traceback),
...
@slash.slash_command(name="edit_problem", description = "edit a problem", options = [
    ...
    Option(name = "new_answer", description="the new answer",
           type=OptionType.STRING,required=False)])
async def edit_problem(inter,problem_id,new_question=None,new_answer=None,guild_id="null"):
    ...
    try:
        ...
        if not problem.is_author(inter.author):
            await inter.reply(embed=ErrorEmbed(
...
@slash.slash_command(name="show_problem_info", description = "Show problem info", options=[
    ...
    Option(name="is_guild_problem", 
           ...
           type=OptionType.BOOLEAN, required=False)])
async def show_problem_info(inter, problem_id, show_all_data=False, raw=False,is_guild_problem=False):
    ...
    if True:  
        if is_guild_problem and guild_id is None:
            ...
            return
        problem = await main_cache.get_problem(str(inter.guild.id) if is_guild_problem else "null",
                                         str(problem_id))
        ...
        if show_all_data:
            if not ((problem.is_author(inter.author) or inter.author.id not in bot.trusted_users or (
                is_guild_problem and inter.author.guild_permissions.administrator == True))): #Check for sufficient permissions
        ...
        if raw:
            await inter.reply(embed=SuccessEmbed(str(problem.convert_to_dict())), ephemeral=True)
            return
...
async def generate_new_problems(inter, num_new_problems_to_generate):
    ...
    if num_new_problems_to_generate > 200:
        await inter.reply("You are trying to create too many problems. Try something smaller than or equal to 200.",ephemeral=True)
...
async def delallbotproblems(inter):
    ...
    await inter.reply(embed=SimpleEmbed("",description="Attempting to delete bot problems"),ephemeral=True) #may get rid of later? :)
...
async def submit_problem(inter, answer, question, guild_question=False):
    ...
    if guild_question:
        guild_id = inter.guild.id
        if guild_id is None:
            await inter.reply(embed=ErrorEmbed("You need to be in the guild to make a guild question!"))
            return

        if guild_id not in main_cache.get_guilds():
            main_cache.add_empty_guild(guild_id)
        elif len(main_cache.get_guild_problems(inter.guild)) >= guild_maximum_problem_limit and guild_question:

            await inter.reply(embed=ErrorEmbed("You have reached the guild math problem limit."))
            return

        while True:

            problem_id = generate_new_id()
            if problem_id not in [problem.id for problem in main_cache.get_guild_problems(inter.guild)]:
                break
        ...
        await inter.reply(embed=SuccessEmbed("You have successfully made a math problem!",
...
async def check_answer(inter,problem_id,answer, checking_guild_problem=False):
    ...
    if inter.guild != None and inter.guild.id not in main_cache.get_guilds():
        main_cache.add_empty_guild(inter.guild)
    try:
        problem = main_cache.get_problem(inter.guild.id if checking_guild_problem else "null", str(problem_id))
        if problem.is_solver(inter.author):
            await inter.reply(embed=ErrorEmbed("You have already solved this problem!",
                                             custom_title="Already solved."),
                            ephemeral = True)
            return
    except KeyError:
        await inter.reply(embed=ErrorEmbed("This problem doesn't exist!",
    ...
    if not problem.check_answer(answer):
        ...
        await inter.reply(embed=SuccessEmbed("",successTitle="You answered this question correctly!"), ephemeral=True)
...
@slash.slash_command(name="list_all_problems", description = "List all problems stored with the bot",
                     options=[
                         ...
                         Option(name="show_only_guild_problems", description="Whether to only show guild problems",
                                type=OptionType.BOOLEAN, required=False)])
async def list_all_problems(inter, show_solved_problems=False,show_guild_problems=True,show_only_guild_problems=False):
    ...
    if mathProblems.keys() == []:
        await inter.reply(embed=ErrorEmbed("There aren't any problems! You should add one!"), ephemeral=True)
        return
...
async def set_vote_threshold(inter,threshold):
    ...
    try:
        ...
        return
    if inter.author.id not in bot.trusted_users:
        await inter.reply(embed=ErrorEmbed("You aren't allowed to do this!"), ephemeral=True)
        return
    if threshold <1:
        await inter.reply(embed=ErrorEmbed(
    ...
    await inter.reply(embed=SuccessEmbed(f"The vote threshold has successfully been changed to {threshold}!"),
                    ephemeral=True)
...
async def vote(inter, problem_id,is_guild_problem=False):
    ...
    try:
        ...
        if problem.is_voter(inter.author):
            await inter.reply(embed=ErrorEmbed("You have already voted for the deletion of this problem!"), ephemeral=True)
            return
    except problems_module.ProblemNotFound:
        await inter.reply(embed=ErrorEmbed("This problem doesn't exist!"), ephemeral=True)
        return
    ...
    await inter.reply(embed=SuccessEmbed(string_to_print, title = "YouSuccessfully voted"), ephemeral=True)
    if problem.get_num_voters() >= vote_threshold:
        main_cache.remove_problem(guild_id=problem.guild_id, problem_id = problem.id)
        await inter.reply(embed=SimpleEmbed("This problem has surpassed the threshold and has been deleted!"), ephemeral=True)
...
async def unvote(inter, problem_id,is_guild_problem=False):
    ...
    if inter.guild != None and inter.guild.id not in main_cache.get_guilds():
        main_cache.add_empty_guild(inter.guild)
    try:
        problem = main_cache.get_problem(inter.guild.id if is_guild_problem else "null",
                                         problem_id=str(problem_id))
        if not problem.is_voter(inter.author):
            await inter.reply(embed=ErrorEmbed("You can't unvote since you are not voting."), ephemeral=True)
            return
    except problems_module.ProblemNotFound:
        await inter.reply(embed=ErrorEmbed("This problem doesn't exist!"), ephemeral=True)
        return
...
async def delete_problem(inter, problem_id,is_guild_problem=False):
    ...
    if is_guild_problem:
        ...
        if problem_id not in main_cache.get_guild_problems(inter.guild).keys():
            await inter.reply(embed=ErrorEmbed("That problem doesn't exist."), ephemeral=True)
            return
        if not (inter.author.id in bot.trusted_users or not (
    ...
    if not (inter.author.id in bot.trusted_users or not
            ...
            or inter.author.guild_permissions.administrator): #Not
        await inter.reply(embed=ErrorEmbed("Insufficient permissions!"), ephemeral=True)
        return
    main_cache.remove_problem("null", problem_id)
    await inter.reply(embed=SuccessEmbed(f"Successfully deleted problem #{problem_id}!"), ephemeral=True)
...
You must also share a server with the new trusted user."""
    ...
    if inter.author.id not in bot.trusted_users:
        await inter.reply(embed=ErrorEmbed("You aren't a trusted user!"), ephemeral=True)
        return
    if user.id in bot.trusted_users:
        await inter.reply(embed=ErrorEmbed(f"{user.name} is already a trusted user!"),
                        ephemeral=True)
        return
    bot.trusted_users.append(user.id)
    await inter.reply(embed=ErrorEmbed(f"Successfully made {user.nick} a trusted user!"), ephemeral=True) 
...
async def remove_trusted_user(inter,user):
    ...
    if user.id not in bot.trusted_users:
        await inter.reply(embed=ErrorEmbed(f"{user.name} isn't a trusted user!", ephemeral=True))
        return
    bot.trusted_users.pop(user.id)
    await inter.reply(embed=ErrorEmbed(f"Successfully made {user.nick} no longer a trusted user!"), ephemeral=True) 
...
async def ping(inter):
    ...
    await inter.reply(embed=SuccessEmbed(f"Pong! My latency is {round(bot.latency*1000)}ms."), ephemeral=True)
...
async def what_is_vote_threshold(inter):
    ...
    await inter.reply(embed=SuccessEmbed(f"The vote threshold is {vote_threshold}."),ephemeral=True)
