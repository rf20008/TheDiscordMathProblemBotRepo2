...
async def remove_trusted_user(ctx,user):
  ...
  user_id = member.id
  if user_id == None:
    await ctx.send(f"{user.name} isn't a valid nickname of a user!", hidden=True)
    return
  #user_id = e.id
  ...
  await ctx.send(f"Successfully made {user.name} no longer a trusted user!", hidden=True) 
